var projectArray=[
    {
        name: "Docs Reminder",
        disp: "Custom Reminder Android App for Remider of Documents and Bill Payment.",
        language: "JAVA, XML, Android",
        purpose: "Summer Intership (Live)",
        link: "https://play.google.com/store/apps/details?id=com.bdf.doc.testrreminderui&hl=en",
        pImage: "pBg.jpg"
    },
    {
        name: "EDcrypt",
        disp: "Password Encryption and Decryption Android App to Store Password Securly anyware.",
        language: "JAVA, XML, Android",
        purpose: "Self Learning",
        link: "https://github.com/XenonAman/EDcrypt",
        pImage: "pBg.jpg"
    },
    {
        name: "Calculator",
        disp: "Calculator Android App similar to Samsung Calculator.",
        language: "JAVA, XML, Android",
        purpose: "Self Learning",
        link: "https://github.com/XenonAman/CalculatorSaud",
        pImage: "pBg.jpg"
    },
    {
        name: "E-Cell StudStart",
        disp: "Web portal for E-Cell VSIT, Startup Jobs site for Freshers Studesnts.",
        language: "MERN Stack",
        purpose: "Third Year Project",
        link: "https://github.com/ashutoshtrack/Proj-SAS",
        pImage: "pBg.jpg"
    },
    {
        name: "ToDo List WebApp",
        disp: "ToDo List WebApp with Login and Signin for maintaining the self ToDo's.",
        language: "MERN Stack",
        purpose: "Intership Selection",
        link: "https://github.com/XenonAman/ToDoList_ReactWebApp",
        pImage: "pBg.jpg"
    },
    {
        name: "Bizcon 2017 & 2018 Website",
        disp: "E-Cell annual fest BIZCON  2018 and 2019 Informatic website.",
        language: "HTML, CSS, Bootstrap, JS",
        purpose: "E-Cell | VSIT Fest",
        link: "http://ecell.vsit.edu.in/bizcon/",
        pImage: "pBg.jpg"
    },
    {
        name: "Website Templates",
        disp: "Self Learning and getting grip on FrontEnd Side and Other Contributions.",
        language: "HTML, CSS, Bootstrap, JS",
        purpose: "Self Learning & Templates",
        link: "https://github.com/XenonAman/Websites",
        pImage: "pBg.jpg"
    },
    {
        name: "Excel Web Automation",
        disp: "Web Scarping Excel Macros to reduce time and Efford in Curent Project Porcess.",
        language: "VBA",
        purpose: "Project Process @ Infosys ltd.",
        link: "https://github.com/XenonAman?tab=repositories",
        pImage: "pBg.jpg"
    },
    {
        name: "Other Contributions",
        disp: "Self Learning, getting grip on Different Languages and Contributions on Others Projects.",
        language: "____________",
        purpose: "Self Learning",
        link: "https://github.com/XenonAman?tab=repositories",
        pImage: "pBg.jpg"
    },
]

var skillArray = [
    {
        skillIcon: "uipath.jpg",
        skill : "UiPath",
        level : 95
    },
    {
        skillIcon: "vb.jpg",
        skill : "VB.Net",
        level : 70
    },
    {
        skillIcon: "py.jpg",
        skill : "Python",
        level : 65
    },
    {
        skillIcon: "cs.jpg",
        skill : "C#",
        level : 70
    },
    {
        skillIcon: "java.jpg",
        skill : "Java",
        level : 65
    },
    {
        skillIcon: "js.jpg",
        skill : "JavaScript",
        level : 70
    },
    {
        skillIcon: "php.jpg",
        skill : "PHP",
        level : 60
    },
    {
        skillIcon: "c++.jpg",
        skill : "C ++",
        level : 55
    },
    {
        skillIcon: "njs.jpg",
        skill : "Node JS",
        level : 70
    },
    {
        skillIcon: "ejs.jpg",
        skill : "Express JS",
        level : 70
    },
    {
        skillIcon: "rjs.jpg",
        skill : "React JS",
        level : 70
    },
    {
        skillIcon: "mdb.jpg",
        skill : "MongoDB",
        level : 80
    },
    {
        skillIcon: "sql.jpg",
        skill : "MySQL",
        level : 90
    },
    {
        skillIcon: "access.jpg",
        skill : "Microsoft Access",
        level : 75
    },
    {
        skillIcon: "android.jpg",
        skill : "Android Development",
        level : 45
    },
    {
        skillIcon: "html.jpg",
        skill : "HTML 5",
        level : 95
    },
    {
        skillIcon: "css.jpg",
        skill : "CSS 3",
        level : 90
    },
    {
        skillIcon: "bs.jpg",
        skill : "Bootstrap",
        level : 75
    },
    {
        skillIcon: "wp.jpg",
        skill : "Wordpress",
        level : 40
    },
    {
        skillIcon: "selenium.jpg",
        skill : "Selenium",
        level : 70
    },
    {
        skillIcon: "excelMacro.jpg",
        skill : "Excel Macro Scripting",
        level : 70
    },
    {
        skillIcon: "git.jpg",
        skill : "GitHub",
        level : 75
    },
    {
        skillIcon: "ms.jpg",
        skill : "Microsoft Office",
        level : 85
    },
    {
        skillIcon: "ps.jpg",
        skill : "Adobe Photoshop CS6",
        level : 80
    },
    {
        skillIcon: "ai.jpg",
        skill : "Adobe Illustrator",
        level : 60
    },
    {
        skillIcon: "ae.jpg",
        skill : "Adobe After Effects",
        level : 65
    }
]


var cCertiArray = [
    {
        cCertiImg: "certiRPADev.jpg",
        cCertiName: "Infosys Certified RPA Developer"
    },
    {
        cCertiImg: "certiRPAAsc.jpg",
        cCertiName: "Infosys Certified UiPath Associate"
    },
    {
        cCertiImg: "RPAfoundation.jpg",
        cCertiName: "RPA Developer Foundation"
    },
    {
        cCertiImg: "RPAorch.jpg",
        cCertiName: "UiPath Orchestrator Training"
    },
    {
        cCertiImg: "fullStack.jpg",
        cCertiName: "Full Stack Web Development"
    },
    {
        cCertiImg: "PHP_MySQL.jpg",
        cCertiName: "PHP and MySQL Training"
    },
    {
        cCertiImg: "agile.jpg",
        cCertiName: "Being Agile Aware"
    },
    {
        cCertiImg: "uipathDev.jpg",
        cCertiName: "Developing Automations with UiPath"
    },
]

var pCertiArray = [
    {
        pCertiImg: "dum.png",
        pCertiName: "1st Certificate"
    },
    {
        pCertiImg: "dum.png",
        pCertiName: "1st Certificate"
    },
    {
        pCertiImg: "dum.png",
        pCertiName: "2st Certificate"
    },
]

        // mainArray.forEach((element,index) => {
        //     setTimeout(function(){
        //         document.getElementById("id1").innerHTML = "Skills: " + element.skill;
        //         document.getElementById("id2").innerHTML = "Level: " + element.level;
        //      }, 1000 * index);     
        // });